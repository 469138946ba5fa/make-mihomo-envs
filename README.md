...
* 如需**全局路由**，请将路由器 DHCP 下发的 网关 强制为本机 IP 同时将下发 DNS 为路由器 IP 或任意真实 DNS（如 1.1.1.1, 8.8.8.8 或 223.5.5.5），不要设置为 fake-ip 地址
* 如需**旁路由**，请将路由器或设备 网关 强制为本机 IP 同时将下发 DNS 为路由器 IP 或任意真实 DNS（如 1.1.1.1, 8.8.8.8 或 223.5.5.5），不要设置为 fake-ip 地址
