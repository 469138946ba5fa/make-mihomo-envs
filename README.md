...
A: 这涉及到一个非常经典的问题，我的配置用的fake-ip它使用的是中间人攻击（Man-in-the-Middle, MITM）的应用变种或合法衍生认证网站，也就是说如果，没有对应的伪造证书，那么有些节点就无法使用访问网站，而7890端口使用的是http明文访问涉及不到中间人伪造证书这一过程所以可以直接访问。  
...
tls:
  ...
  sniff: true
```
* 完成以上操作，最后就可以执行脚本创建 mihomo 代理环境+解密https流量版（MITM）
